package ru.job4j.algo.sort;

import org.junit.jupiter.api.Test;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

class QuickSortTest {
    @Test
    void whenSortThenOk() {
        User user1 = new User(1, "11");
        User user2 = new User(1, "22");
        User user3 = new User(3, "33");
        User user4 = new User(-4, "44");
        User user5 = new User(4, "55");
        User user6 = new User(6, "66");
        Comparator<User> comparator = Comparator.comparingInt(User::getId);
        List<User> listUser = new ArrayList<>();
        listUser.add(user3);
        listUser.add(user5);
        listUser.add(user1);
        listUser.add(user4);
        listUser.add(user6);
        listUser.add(user2);
        QuickSort.quickSort(listUser, comparator);
        List<Integer> result = listUser.stream().map(User::getId).toList();
        System.out.println(result);
        assertThat(result).containsExactly(-4, 1, 1, 3, 4, 6);
    }

    @Test
    void whenReverseOrderThenOk() {
        Comparator<Integer> comparator = (a, b) -> b - a;
        List<Integer> list = new ArrayList<>();
        list.add(-2);
        list.add(-8);
        list.add(1);
        list.add(0);
        list.add(10);
        list.add(1);
        list.add(8);
        QuickSort.quickSort(list, comparator);
        System.out.println(list);
        assertThat(list).containsExactly(10, 8, 1, 1, 0, -2, -8);
    }

    @Test
    void whenSortEmptyListThenEmptyList() {
        Comparator<Integer> comparator = Comparator.comparingInt(i -> i);
        List<Integer> list = new ArrayList<>();
        QuickSort.quickSort(list, comparator);
        assertThat(list).isEmpty();
    }

    @Test
    void whenSortSingleElementThenThisElement() {
        Comparator<Integer> comparator = Comparator.comparingInt(i -> i);
        List<Integer> list = new ArrayList<>();
        list.add(42);
        QuickSort.quickSort(list, comparator);
        assertThat(list).containsExactly(42);
    }

    @Test
    void whenSortSortedListThenOk() {
        Comparator<Integer> comparator = (a, b) -> b - a;
        List<Integer> list = new ArrayList<>(List.of(1, 2, 3, 4, 5, 6, 7, 8, 9));
        QuickSort.quickSort(list, comparator);
        assertThat(list).isSortedAccordingTo(comparator);
    }

    @Test
    void whenSortReverseSortedThenOk() {
        Comparator<Integer> comparator = Comparator.comparingInt(i -> i);
        List<Integer> list = new ArrayList<>(List.of(42, 23, 16, 15, 8, 4));
        QuickSort.quickSort(list, comparator);
        assertThat(list).isSortedAccordingTo(comparator);
    }

    @Test
    void whenSortWithDuplicatesThenOk() {
        User user1 = new User(1, "John");
        User user2 = new User(2, "Richard");
        User user3 = new User(3, "Carl");
        User user4 = new User(4, "John");
        User user5 = new User(5, "Michael");
        User user6 = new User(6, "William");
        User user7 = new User(7, "Michael");
        Comparator<User> comparator = Comparator.comparing(User::getName).reversed();
        List<User> listUser = new ArrayList<>();
        listUser.add(user1);
        listUser.add(user2);
        listUser.add(user3);
        listUser.add(user4);
        listUser.add(user5);
        listUser.add(user6);
        listUser.add(user7);
        QuickSort.quickSort(listUser, comparator);
        List<String> result = listUser.stream().map(User::getName).toList();
        System.out.println(result);
        assertThat(result).containsExactly("William", "Richard", "Michael", "Michael", "John", "John", "Carl");
    }

    private static class User {
        private Integer id;
        private String name;

        public User(int id, String name) {
            this.id = id;
            this.name = name;
        }

        public int getId() {
            return id;
        }

        public String getName() {
            return name;
        }
    }
}