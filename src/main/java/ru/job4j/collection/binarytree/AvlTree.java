...
public class AvlTree<T extends Comparable<T>> {
    ...
    public boolean contains(T value) {
        return Objects.nonNull(find(root, value));
    }
    ...
    public T minimum() {
        return Objects.nonNull(root) ? minimum(root) : null;
    }

    private T minimum(Node node) {
        return Objects.nonNull(node.left) ? minimum(node.left) : node.key;
    }

    public T maximum() {
        return Objects.nonNull(root) ? maximum(root) : null;
    }

    private T maximum(Node node) {
        return Objects.nonNull(node.right) ? maximum(node.right) : node.key;
    }
